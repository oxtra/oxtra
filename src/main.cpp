...
int main(int argc, char** argv) {
	...
	spdlog::info("Finished parsing the arguments.");

	//create the elf-object
	const auto elf = elf::Elf(arguments.get_guest_path());
	spdlog::info("Finished reading and parsing elf file.");

	//create the dispatcher
	dispatcher::Dispatcher dispatcher(elf, arguments);
	spdlog::info("Finished creating and initializing various runtime-objects.");
