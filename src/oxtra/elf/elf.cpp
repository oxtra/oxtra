...
void Elf::unpack_file() {
	...
	std::unique_ptr<uint8_t[]> image_buffer = std::unique_ptr<uint8_t[]>(
			reinterpret_cast<uint8_t*>(mmap(reinterpret_cast<void*>(base_address), image_size, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)));

	//validate the base-address
	if (reinterpret_cast<uintptr_t>(image_buffer.get()) != base_address)
	...
	memset(image_buffer.get(), 0, image_size);
	...
	for (size_t i = 0; i < p_c; i++) {
		...
		if (copy_src + copy_size > reinterpret_cast<uintptr_t>(_image_ptr.get()) + _image_size)
			copy_size = reinterpret_cast<uintptr_t>(_image_ptr.get()) + _image_size - copy_src;
...
Elf::Elf(const uint8_t* ptr, size_t size, uintptr_t base_address, size_t exec_pages) {
	...
	_image_size = (size & 0x00000FFFu) > 0 ? size + 0x1000 : size;
	_image_size ^= (_image_size & 0x00000FFFu);

	// allocate the buffer for the image
	_image_ptr = std::unique_ptr<uint8_t[]>(
			reinterpret_cast<uint8_t*>(mmap(reinterpret_cast<void*>(base_address), _image_size,
											PROT_EXEC | PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)));

	// validate the base-address
	if (reinterpret_cast<uintptr_t>(_image_ptr.get()) != base_address)
		throw ElfException(ElfException::base_address_taken);

	// allocate and initialize the page-array
	_page_flags = std::make_unique<uint8_t[]>(_image_size >> 12u);
	for (size_t i = 0; i < (_image_size >> 12u); i++)
		_page_flags[i] = ((i < exec_pages) ? PAGE_EXECUTE : PAGE_WRITE) | PAGE_MAPPED | PAGE_READ;


	// write the buffer to the image
	memcpy(_image_ptr.get(), ptr, size);
}

uint8_t Elf::get_page_flags(uintptr_t vaddr) const {
	//validate the address
	vaddr -= reinterpret_cast<uintptr_t>(_image_ptr.get());
	if (vaddr >= _image_size)
		return 0;
...
size_t Elf::get_size(uintptr_t vaddr, size_t max_page) const {
	...
	max_page += vaddr;
	if (max_page > (_image_size >> 12u))
		max_page = (_image_size >> 12u);
