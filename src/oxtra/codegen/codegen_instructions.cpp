...
void CodeGenerator::translate_ret(const Instruction& inst, riscv_instruction_t* riscv, size_t& count) {
	load_64bit_immediate(reinterpret_cast<uint64_t>(Dispatcher::guest_exit), RiscVRegister::t0, riscv, count, false);
	riscv[count++] = JALR(RiscVRegister::zero, RiscVRegister::t0, 0);
}
