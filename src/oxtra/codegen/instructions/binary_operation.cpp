...
void codegen::BinaryOperation::generate(CodeBatch& batch) const {
	...
	if (src.get_type() != OperandType::imm ||
		...
		source_register = translate_operand(batch, src, nullptr, RiscVRegister::t2, RiscVRegister::t3, false, false, false, false);
	}
	...
	if (dest_value)
		dest_register = translate_operand(batch, dest, &address, dest_register, RiscVRegister::t1, true, false, false, true);
