...
void codegen::Comparable::generate(CodeBatch& batch) const {
	...
	if (src.get_type() != OperandType::imm ||
		...
		source_register = translate_operand(batch, src, nullptr, RiscVRegister::t2, RiscVRegister::t3, false, false, false,
											false);
	}

	// load the destination-register
	auto dest_register = translate_operand(batch, dest, nullptr, RiscVRegister::t0, RiscVRegister::t1, true, false, false, true);
